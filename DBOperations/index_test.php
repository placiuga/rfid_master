...
$userQuery = "SELECT * FROM UserTable 
...
if($userExists) {
    $userIDrequest = "SELECT StudentID FROM UserTable
                      WHERE RFID_UID='$RFID_UID'";
    $result = $conn->query($userIDrequest);
    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $studentID = $row['StudentID'];
    }
}
else if(!$userExists && !$machineExists) {
...
$authQuery = "SELECT * FROM MachineAuthorizations
